...
public class LitePager extends ViewGroup implements Runnable {
    ...
    private static final float DEFAULT_TOP_ROTATION = 0;
    private static final float DEFAULT_TOP_ELEVATION = 0.3F;
    ...
    private static final float DEFAULT_MIDDLE_ALPHA = .8F;
    private static final float DEFAULT_MIDDLE_ELEVATION = 0.5F;
    private static final float DEFAULT_LEFT_ROTATION = 10F;


    private static final float DEFAULT_BOTTOM_SCALE = .6F;
    private static final float DEFAULT_BOTTOM_ALPHA = .6F;
    private static final float DEFAULT_BOTTOM_ELEVATION = 0.8F;
    private static final float DEFAULT_RIGHT_ROTATION = -10F;
    ...
    private float mTopRotation, mLeftRotation, mRightRotation;//Y轴旋转角度
    private float mTopElevation, mMiddleElevation, mBottomElevation;//阴影透明度
    private int leftChild1Index, leftChild2Index;//左1，2view序号
    private int rightChild1Index, rightChild2Index;//右1，2view序号
    private int topChildIndex;//中间view序号
    ...
    private ViewList<Integer> viewIndexList = new ViewList();//存放被隐藏的view序号
    private ArrayList<View> viewList=new ArrayList<>();//存放被添加进来的view
...
//        viewCount = a.getInteger(R.styleable.LitePager_viewCount, 0);
        ...
        mTopElevation = DEFAULT_TOP_ELEVATION;
        mRightRotation = a.getFloat(R.styleable.LitePager_topRotation, DEFAULT_TOP_ROTATION);
        ...
        mMiddleElevation = DEFAULT_MIDDLE_ELEVATION;
        mLeftRotation = a.getFloat(R.styleable.LitePager_leftRotation, DEFAULT_LEFT_ROTATION);
        ...
        mBottomElevation = DEFAULT_BOTTOM_ELEVATION;
        mRightRotation = a.getFloat(R.styleable.LitePager_rightRotation, DEFAULT_RIGHT_ROTATION);
    ...
    private void fixOverflow() {
        ...
        mLeftRotation = fixOverflowRotation(mLeftRotation);
        mTopScale = fixOverflow(mTopScale);
        mTopAlpha = fixOverflow(mTopAlpha);
        mTopRotation = fixOverflowRotation(mTopRotation);
        mBottomScale = fixOverflow(mBottomScale);
        mBottomAlpha = fixOverflow(mBottomAlpha);
        mRightRotation = fixOverflowRotation(mRightRotation);
    }
    ...
    private float fixOverflowRotation(float value) {
        return value > 20 ? 20 : value < -20 ? -20 : value;
    }
     ...
     */
    public LitePager addViews(int... layouts) {
     ...
     * @param views 目标子View
     */
    public LitePager addViews(View... views) {
        ...
        Log.e("Biver", "addViews: " +viewList.size());
        if(viewCount>5){
            leftChild1Index = 0;
            rightChild1Index = viewCount - 4;
            leftChild2Index = viewCount - 3;
            rightChild2Index = viewCount - 2;
            topChildIndex = viewCount - 1;
        }
    ...
    public void setSelection(int index) {
        ...
        if (is5Child()) {
            ...
            if(index==leftChild1Index){
                isNeedPlayTwice = index != mSelectedIndex;
                end = isHorizontal() ? getWidth() : getHeight();
            }else if(index==rightChild1Index){
                isNeedPlayTwice = index != mSelectedIndex;
                end = isHorizontal() ? -getWidth() : -getHeight();
            }else if(index==leftChild2Index){
                end = isHorizontal() ? getWidth() : getHeight();
            }else if(index==rightChild2Index){
                end = isHorizontal() ? -getWidth() : -getHeight();
            }else{
                return;
            }
        ...
        if (Math.abs(mOffsetPercent) > .5F) {
            if (!isReordered) {
                if (is5Child()) {
                    ...
                    if (mOffsetPercent > 0) {
                        reOrderByMax(1, viewIndexList,leftChild1Index, rightChild2Index, rightChild1Index, topChildIndex, leftChild2Index);
                    } else {
                        reOrderByMax(0, viewIndexList,leftChild2Index, leftChild1Index, topChildIndex, rightChild1Index, rightChild2Index);
                    }
                } else if (is4Child()) {
                    if (mOffsetPercent > 0) {
                        reOrder(1, 2, 3, 0);
    ...
    private List<View> mTempViewList = new ArrayList<>();
    ...
    private void reOrderByMax(int by,ViewList<Integer> list, int... indexes) {
        mTempViewList.clear();
        int count=0;
        for (int index : indexes) {
            if (index >= getChildCount()) {
            ...
            if(count==2&&by==0){
                for (int data:list){
                    mTempViewList.add(getChildAt(data));
                    Log.e("Biver", "reOrderByMax0: " +mTempViewList.size());
                }
            }else if(count==0&&by==1){
                for (int data:list){
                    mTempViewList.add(getChildAt(data));
                    Log.e("Biver", "reOrderByMax1: " +mTempViewList.size());
                }
            }
            mTempViewList.add(getChildAt(index));
            count++;
    ...
    private void updateFromAndTo() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (is5Child()) {
                ...
                if(lp.from==leftChild1Index){
                    lp.to=mOffsetPercent>0?leftChild2Index:leftChild1Index+1;
                }else if(lp.from==leftChild1Index+1){
                    lp.to=mOffsetPercent>0?leftChild1Index:leftChild1Index+2;
                }else if(lp.from==rightChild1Index-1){
                    lp.to=mOffsetPercent>0?rightChild1Index-2:rightChild1Index;
                }else if(lp.from==rightChild1Index){
                    lp.to=mOffsetPercent>0?rightChild1Index-1:rightChild2Index;
                }else if(lp.from==leftChild2Index){
                    lp.to=mOffsetPercent>0?topChildIndex:leftChild1Index;
                }else if(lp.from==rightChild2Index){
                    lp.to=mOffsetPercent>0?rightChild1Index:topChildIndex;
                }else if(lp.from==topChildIndex){
                    lp.to=mOffsetPercent>0?rightChild2Index:leftChild2Index;
                }else{
                    lp.to=mOffsetPercent>0?lp.from-1:lp.from+1;
                }
//                switch (lp.from) {
//                    case 0:
//                        lp.to = mOffsetPercent > 0 ? 3 : 1;
//                        break;
//                    case 1:
...
//                        break;
//                    case 3:
//                        lp.to = mOffsetPercent > 0 ? 5 : 0;
//                        break;
//                    case 4:
//                        lp.to = mOffsetPercent > 0 ? 2 : 5;
//                        break;
//                    case 5:
//                        lp.to = mOffsetPercent > 0 ? 4 : 3;
//                        break;
//                    default:
//                        break;
//                }
            } else if (is4Child()) {
//                Log.e("Biver", "updateFromAndTo: "+mOffsetPercent );
                switch (lp.from) {
                    case 0:
                        lp.to = mOffsetPercent > 0 ? 3 : 1;
                        break;
                    case 1:
                        ...
                        break;
                    case 2:
                        lp.to = mOffsetPercent > 0 ? 1 : 3;
                        break;
                    case 3:
                        lp.to = mOffsetPercent > 0 ? 2 : 0;
                        break;
                    default:
                        break;
                }
    ...
    private boolean handleActionUp(float x, float y) {
        ...
        if (Math.abs(offsetX) < mTouchSlop && Math.abs(offsetY) < mTouchSlop) {
            ...
            if (hitView != null) {
                if (indexOfChild(hitView) == (is5Child()?4:isMaxChild()? topChildIndex : is4Child()?3:2)) {
    ...
    private int getBaselineByChild(View child) {
        ...
        if (is5Child()) {
            ...
            baseLine = getHorizontalBaseLineByMaxChild(child);
            updateAlphaAndScaleByMaxChild(child);
        } else {
            baseLine = isHorizontal() ? getHorizontalBaseLine(child) : getVerticalBaseLine(child);
            if(is4Child()){
                updateAlphaAndScaleBy4Child(child);
            }else{
    ...
    private int getHorizontalBaseLine(View child) {
        ...
        int baseLine = 0;
        return is4Child()?getBaseLineBy4Child(child, baseLineLeft, baseLineCenterX, baseLineRight):getBaseLine(child, baseLineLeft, baseLineCenterX, baseLineRight);
    }
    ...
    private int getBaseLineBy4Child(View child, int start, int middle, int end) {
        int baseLine = 0;
        ...
        switch (lp.from) {
            ...
            case 2:
                switch (lp.to) {
                    case 1:
                        baseLine = end + (int) ((end - middle) * -mOffsetPercent/*因为是反方向*/);
                        break;
                    case 3:
                        baseLine = end + (int) ((end - middle) * mOffsetPercent);
                        break;
            ...
            case 1:
                switch (lp.to) {
                    case 0:
                        baseLine = middle + (int) ((end - middle) * -mOffsetPercent/*因为是反方向*/);
                        break;
                    case 2:
                        baseLine = middle + (int) ((middle - start) * -mOffsetPercent);
                        break;
    ...
    private void updateAlphaAndScale(View child) {
        LayoutParams lp = (LayoutParams) child.getLayoutParams();
        float abs = Math.abs(mOffsetPercent);
        Log.e("Biver", "updateAlphaAndScale: ");
        switch (lp.from) {
            case 0:
                switch (lp.to) {
                    case 0:
                    case 1:
                        ...
                        lp.elevation=mMiddleElevation;
                        if (lp.from == 0 && abs > 0) {
                            lp.rotation = mLeftRotation - (mLeftRotation * abs) + (mRightRotation * abs);
                        } else if (lp.from == 1 && abs > 0) {
                            lp.rotation = mRightRotation - (mRightRotation * abs) + (mLeftRotation * abs);
                        }
                        break;
                    case 2:
                        float alphaProgress;
                        ...
                        lp.elevation=mMiddleElevation + (mTopElevation - mMiddleElevation) * mOffsetPercent;
                        if (mOffsetPercent != 0) {
                            lp.rotation = mLeftRotation - mLeftRotation * abs;
                        }
                        break;
                }
                break;
            case 1:
                switch (lp.to) {
                    ...
                    case 2:
                        ...
                        lp.elevation=mMiddleElevation+(mTopElevation-mMiddleElevation)*-mOffsetPercent;
                        if (mOffsetPercent != 0) {
                            lp.rotation = mRightRotation - mRightRotation * abs;
                        }
                        break;
                }
                break;
            case 2:
                ...
                lp.scale = mTopScale - (mTopScale - mMiddleScale) * abs;
                lp.elevation=mTopElevation-(mTopElevation-mMiddleElevation)*abs;
                if (lp.to == 0 && mOffsetPercent != 0) {
                    lp.rotation = mLeftRotation * abs;
                } else if (lp.to == 1 && mOffsetPercent != 0) {
                    lp.rotation = mRightRotation * abs;
                }
                break;
        }
    }

    private void updateAlphaAndScaleBy4Child(View child) {
        float abs = Math.abs(mOffsetPercent);
        LayoutParams lp = (LayoutParams) child.getLayoutParams();
        switch (lp.from) {
            case 0:
                switch (lp.to) {
                    case 1:
                        ...
                        lp.scale = mMiddleScale;
                        lp.elevation=mMiddleElevation;
                        lp.rotation = mLeftRotation - mLeftRotation * -mOffsetPercent;
                        break;
                    case 3:
                        float alphaProgress;
                        if (mOffsetPercent > .5F) {
            ...
            case 1:
                switch (lp.to) {
                    ...
                    case 2:
                        ...
                        lp.elevation=mMiddleElevation;
                        if (lp.to == 0 && mOffsetPercent != 0) {
                            lp.rotation = mLeftRotation * Math.abs(mOffsetPercent);
                        } else if (lp.to == 2 && mOffsetPercent != 0) {
                            lp.rotation = mRightRotation * Math.abs(mOffsetPercent);
                        }
                        break;
                    default:
//                        setAsBottom(child);
...
//                        setAsBottom(child);
                        ...
                        lp.scale = mMiddleScale;
                        lp.elevation=mMiddleElevation;
                        lp.rotation = mRightRotation - mRightRotation * mOffsetPercent;
                        break;
                    case 3:
                        float alphaProgress;
                        //要在后半段才开始改变透明度
...
//                        setAsBottom(child);
            ...
            case 3:
                ...
                lp.elevation = mTopElevation - (mTopElevation - mMiddleElevation) * Math.abs(mOffsetPercent);
                if (lp.to == 0 && mOffsetPercent != 0) {
                    lp.rotation = mLeftRotation * Math.abs(mOffsetPercent);
                } else if (lp.to == 2 && mOffsetPercent != 0) {
                    lp.rotation = mRightRotation * Math.abs(mOffsetPercent);
    ...
    private int getHorizontalBaseLineByMaxChild(View child) {
        return getBaseLineByMaxChild(child, getWidth() / 6);
    }

    private int getBaseLineByMaxChild(View child, int itemDistance) {
        int baseLine = 0;
        int child0Line, child2Line, child3Line, child4Line, child5Line;
        child0Line = itemDistance;
        child3Line = itemDistance * 2;
        child5Line = itemDistance * 3;
        child4Line = itemDistance * 4;
        child2Line = itemDistance * 5;
        LayoutParams lp = (LayoutParams) child.getLayoutParams();
        int offset = (int) (itemDistance * mOffsetPercent);
        Log.e("Biver", "getBaseLineByMaxChild: from" +lp.from+" to"+lp.to);
        if(lp.from==leftChild1Index){
            if(lp.to==leftChild1Index+1){
                baseLine = child0Line - offset * 2;
            }else if(lp.to==leftChild2Index){
                baseLine = child0Line + offset;
            }else{
                baseLine=child0Line;
            }
        }else if(lp.from==leftChild1Index+1){
            if(lp.to==leftChild1Index){
                baseLine = child5Line - offset * 2;
            }else if(lp.to==rightChild1Index){
                baseLine = child5Line - offset * 2;
            }else{
                baseLine=child5Line;
            }
        }else if(lp.from==rightChild1Index-1){
            if(lp.to==rightChild1Index){
                baseLine = child5Line - offset * 2;
            }else if(lp.to==leftChild1Index){
                baseLine=child5Line-offset*2;
            }else{
                baseLine=child5Line;
            }
        }else if(lp.from==rightChild1Index){
            if(lp.to==rightChild1Index-1){
                baseLine=child2Line-offset*2;
            }else if(lp.to==rightChild2Index){
                baseLine = child2Line + offset;
            }else{
                baseLine = child2Line;
            }
        }else if(lp.from==leftChild2Index){
            if(lp.to==leftChild1Index||lp.to==topChildIndex){
                baseLine = child3Line + offset;
            }else {
                baseLine = child3Line;
            }
        }else if(lp.from==rightChild2Index){
            if(lp.to==rightChild1Index||lp.to==topChildIndex){
                baseLine=child4Line+offset;
            }else{
                baseLine=child4Line;
            }
        }else if(lp.from==topChildIndex){
            if(lp.to==leftChild2Index||lp.to==rightChild2Index){
                baseLine=child5Line+offset;
            }else{
                baseLine=child5Line;
            }
        }else{
    ...
    private void updateAlphaAndScaleBy5Child(View child) {
        ...
        switch (lp.from) {
            ...
            case 4:
                ...
                lp.scale = mTopScale - (mTopScale - mMiddleScale) * Math.abs(mOffsetPercent);
                lp.elevation = mTopElevation - (mTopElevation - mMiddleElevation) * Math.abs(mOffsetPercent);
                if (lp.to == 2 && mOffsetPercent != 0) {
                    lp.rotation = mLeftRotation * Math.abs(mOffsetPercent);
                } else if (lp.to == 3 && mOffsetPercent != 0) {
                    lp.rotation = mRightRotation * Math.abs(mOffsetPercent);
                }
                break;
        }
    ...
    private void updateAlphaAndScale2(LayoutParams lp, float offsetPercent) {
        ...
        switch (lp.to) {
            ...
            case 1:
                ...
                lp.elevation = mMiddleElevation + (mMiddleElevation - mBottomElevation) * offsetPercent;
                break;
            //2 to 4||3 to 4    角度随着偏移值逐渐变为0
            case 4:
                lp.alpha = mMiddleAlpha + (mTopAlpha - mMiddleAlpha) * alphaProgress;
                lp.scale = mMiddleScale + (mTopScale - mMiddleScale) * offsetPercent;
                lp.elevation = mMiddleElevation + (mTopElevation - mMiddleElevation) * offsetPercent;
                if (lp.from == 2 && offsetPercent != 0) {
                    lp.rotation = mLeftRotation - mLeftRotation * offsetPercent;
                } else if (lp.from == 3 && offsetPercent != 0) {
                    lp.rotation = mRightRotation - mRightRotation * offsetPercent;
                }
                break;
            default:
                lp.alpha = mMiddleAlpha;
                lp.scale = mMiddleScale;
                lp.rotation = mTopRotation;
                lp.elevation = mMiddleElevation;
        }
    }
    ...
    private void updateAlphaAndScaleBy5Child(View child, LayoutParams lp, float offsetPercent) {
        switch (lp.to) {
            //0 to 1 || 1 to 0      角度转换为对方的角度
            case 0:
            case 1:
                setAsBottomBy5Child(child);
                lp.alpha = mBottomAlpha;
                lp.scale = mBottomScale;
                lp.elevation = mBottomElevation;
                float abs = Math.abs(offsetPercent);
                if (lp.from == 0 && abs > 0) {
                    lp.rotation = mLeftRotation - (mLeftRotation * abs) + (mRightRotation * abs);
                } else if (lp.from == 1 && abs > 0) {
                    lp.rotation = mRightRotation - (mRightRotation * abs) + (mLeftRotation * abs);
            ...
            default:
                ...
                lp.elevation = mBottomElevation + (mMiddleElevation - mBottomElevation) * offsetPercent;
                break;
        }
    }

    private void updateAlphaAndScaleByMaxChild(View child) {
        LayoutParams lp = (LayoutParams) child.getLayoutParams();
        if(lp.from==leftChild1Index){
            updateAlphaAndScaleByMaxChild(child, lp, mOffsetPercent);
        }else if(viewIndexList.contains(lp.from)){
            updateAlphaAndScaleByMaxChild(child, lp, -mOffsetPercent);
        }else if(lp.from==rightChild1Index){
            updateAlphaAndScaleByMaxChild(child, lp, -mOffsetPercent);
        }else if(lp.from==leftChild2Index){
            updateMiddleAlphaAndScale2ByMaxChild(lp, mOffsetPercent);
        }else if(lp.from==rightChild2Index){
            updateMiddleAlphaAndScale2ByMaxChild(lp, -mOffsetPercent);
        }else if(lp.from==topChildIndex){
            float absOffsetPercent = Math.abs(mOffsetPercent);
            ...
            lp.scale = mTopScale - (mTopScale - mMiddleScale) * Math.abs(mOffsetPercent);
            lp.elevation = mTopElevation - (mTopElevation - mMiddleElevation) * Math.abs(mOffsetPercent);
            if (lp.to == leftChild2Index && mOffsetPercent != 0) {
                lp.rotation = mLeftRotation * Math.abs(mOffsetPercent);
            } else if (lp.to == rightChild2Index && mOffsetPercent != 0) {
                lp.rotation = mRightRotation * Math.abs(mOffsetPercent);
            }
        }
    }

    private void updateMiddleAlphaAndScale2ByMaxChild(LayoutParams lp, float offsetPercent) {
        float alphaProgress;
        if (Math.abs(offsetPercent) > .5F) {
            alphaProgress = (Math.abs(offsetPercent) - .5F) * 2;
        } else {
        ...
        if(lp.to==leftChild1Index||lp.to==rightChild1Index){
            lp.alpha = mMiddleAlpha + (mMiddleAlpha - mBottomAlpha) * -alphaProgress;
            lp.scale = mMiddleScale + (mMiddleScale - mBottomScale) * offsetPercent;
            lp.elevation = mMiddleElevation + (mMiddleElevation - mBottomElevation) * offsetPercent;
        }else if(lp.to==topChildIndex){
            lp.alpha = mMiddleAlpha + (mTopAlpha - mMiddleAlpha) * alphaProgress;
            lp.scale = mMiddleScale + (mTopScale - mMiddleScale) * offsetPercent;
            lp.elevation = mMiddleElevation + (mTopElevation - mMiddleElevation) * offsetPercent;
            if (lp.from == leftChild2Index && offsetPercent != 0) {
                lp.rotation = mLeftRotation - mLeftRotation * offsetPercent;
            } else if (lp.from == rightChild2Index && offsetPercent != 0) {
                lp.rotation = mRightRotation - mRightRotation * offsetPercent;
            }
        }else{
            lp.alpha = mMiddleAlpha;
    ...
    private void updateAlphaAndScaleByMaxChild(View child, LayoutParams lp, float offsetPercent) {
        float abs = Math.abs(offsetPercent);
        if(lp.to==0){
            setAsBottomByMaxChild(child);
            lp.alpha = mBottomAlpha;
            lp.scale = mBottomScale;
            lp.elevation = mBottomElevation;
            if (abs > 0) {
        ...
        }else if(lp.to==leftChild1Index+1){
            setAsBottomByMaxChild(child);
            if (lp.from == 0 && abs > 0) {
                lp.rotation = mLeftRotation - mLeftRotation * abs;
            }
        }else if(lp.to==rightChild1Index-1){
            setAsBottomByMaxChild(child);
            lp.rotation = mRightRotation - mRightRotation * abs;
        }else if(lp.to==rightChild1Index){
            setAsBottomByMaxChild(child);
            ...
            lp.elevation = mBottomElevation;
            lp.rotation = mRightRotation * abs;
        }else if(lp.to==leftChild2Index||lp.to==rightChild2Index){
            float alphaProgress;
            if (offsetPercent > .5F) {
                alphaProgress = (offsetPercent - .5F) * 2;
            } else {
     ...
     * 把目标子View放置到视图层级最底部
     */
    private void setAsBottomByMaxChild(View target) {
        //先确定现在在哪个位置
    ...
    private void updateChildParamsAndLayout(View child, int baseLine) {
        LayoutParams lp = (LayoutParams) child.getLayoutParams();
        ViewGroup childViewGroup =(ViewGroup) ((ViewGroup) child).getChildAt(0) ;

//        childViewGroup.getChildAt(0).setAlpha(lp.alpha);
        child.setScaleX(lp.scale);
        child.setScaleY(lp.scale);
        child.setRotationY(lp.rotation);
        if(childViewGroup.getChildCount()>1){
            childViewGroup.getChildAt(childViewGroup.getChildCount() - 1).setAlpha(lp.elevation);
        }
        Paint paint=new Paint();
        paint.setAntiAlias(true);   //抗锯齿
        paint.setDither(true);      //防抖动
        paint.setFilterBitmap(true);//bitmap抗锯齿
        child.setLayerPaint(paint);
        child.setLayerType(View.LAYER_TYPE_HARDWARE,null);
        childViewGroup.setLayerPaint(paint);
        childViewGroup.setLayerType(View.LAYER_TYPE_HARDWARE,null);
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        ...
        Log.e("Biver", "addView123: " +viewCount);
        if (viewCount == 3) {
            if (childCount < 2) {
                lp.alpha = mBottomAlpha;
                lp.scale = mBottomScale;
                ...
                if (childCount == 0) {
                    //左1
                    lp.rotation = mLeftRotation;
                } else {
                    //右1
                    lp.rotation = mRightRotation;
                }
            } else {
                //中间
                lp.alpha = mTopAlpha;
            ...
            if (childCount < 2) {
                lp.alpha = mBottomAlpha;
                lp.scale = mBottomScale;
                lp.elevation = DEFAULT_BOTTOM_ELEVATION;
                if (childCount == 0) {
                    //左1
                    lp.rotation = mLeftRotation;
                } else {
            ...
            //6..N
            if(childCount==viewCount-1){
                //中1
                lp.alpha = mTopAlpha;
                lp.scale = mTopScale;
                lp.elevation = mTopElevation;
                lp.rotation = mTopRotation;
            }else if(childCount==viewCount-2){
                //右2
                lp.alpha = mMiddleAlpha;
                lp.scale = mMiddleScale;
                lp.elevation = mMiddleElevation;
                lp.rotation = mRightRotation;
            }else if(childCount==viewCount-3){
                //左2
                lp.alpha = mMiddleAlpha;
                lp.scale = mMiddleScale;
                lp.elevation = mMiddleElevation;
                lp.rotation = mLeftRotation;
            }else if(childCount==viewCount-4){
                //右1
                lp.alpha = mBottomAlpha;
                lp.scale = mBottomScale;
                lp.elevation = mBottomElevation;
                ...
                lp.elevation = mBottomElevation;
                lp.rotation=mTopRotation;
                viewIndexList.addViewByTail(lp.from);
            }
        }
    ...
    public void run() {
        if (mAutoScrollOrientation == SCROLL_ORIENTATION_LEFT) {
            setSelection(is5Child()?2:isMaxChild()?leftChild2Index: 0);
        } else {
            setSelection(is5Child()?2:isMaxChild()?rightChild2Index:isMaxChild()?rightChild1Index:is4Child()?2:1);
        }
        if (mAutoScrollEnable) {
     ...
     * 设置最小缩放比例
     */
    public void setBottomScale(float scale) {
        mBottomScale = scale;
        if (getChildCount() < 5) {
            mMiddleScale = scale;
        }
     ...
     * 设置最大缩放比例
     */
    public void setTopScale(float scale) {
        mTopScale = scale;
        requestLayout();
    }
     ...
     * 设置中部缩放比例
     */
    public void setMiddleScale(float scale) {
        mMiddleScale = scale;
        requestLayout();
    }
    ...
    public static abstract class Adapter<V extends View> {
        ...
        protected abstract V onCreateView(ViewGroup parent);

        protected abstract void onBindView(V v, int position);
